...
export default function Students() {
  ...
  const inputCls = 'rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none';
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 border-b text-slate-600">
              <tr>
                {['STT','Họ tên','Giới tính','Lớp','Quê quán','Phòng','Vi phạm',''].map(h => (
                ...
                <tr key={s.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-3">
                      ...
                      <button onClick={() => setAssignTarget(s)} className="text-emerald-600 hover:underline text-xs">Xếp phòng</button>
                      <button onClick={() => setViolationTarget(s)} className="text-orange-500 hover:underline text-xs">Vi phạm</button>
                      <button onClick={() => handleDelete(s)} className="text-red-500 hover:underline text-xs">Xóa</button>
                    </div>
                  </td>
                </tr>
              ))}
